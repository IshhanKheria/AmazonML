...
class StageLogger:
    ...
    def progress(self, stage: str, done: int, total: int, extra: dict[str, Any] | None = None) -> None:
        ...
        # Recompute throughput at most once per second from an anchor sample, so
        # bursts of parallel completions do not produce absurd instantaneous rates.
        if now - self._rate_time >= 1.0 and done > self._rate_done:
            self._rate = (done - self._rate_done) / (now - self._rate_time)
            self._rate_time, self._rate_done = now, done
        rate = self._rate if self._rate > 0 else (done / elapsed if elapsed > 0 and done > 0 else 0.0)
